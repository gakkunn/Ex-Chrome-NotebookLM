...
> 💡 This project uses Vite + CRXJS + TypeScript/Preact. Use `npm run dev` for watch builds during development, and `npm run build` for production output in `dist/`.
...
1. Build the extension if necessary:
   ...
   ```bash
   npm run build    # or npm run dev for watch builds
   ```
...
3. Select the built directory (`dist/`). If you are running `npm run dev`, you can also point Chrome to the project root so the watched build updates the files.
...
2. Make necessary changes under `src/`. Run checks or builds as needed.
